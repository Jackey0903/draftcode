...
def _is_http_event(event: dict[str, object]) -> bool:
    return bool(event.get("requestContext") or event.get("httpMethod") or event.get("version") == "2.0")
...
def handler(event: dict[str, object], context: object) -> dict[str, object]:
    event = event or {}
    data_dir = Path(str(event.get("data_dir") or os.getenv("DRAFTCODE_DATA_DIR", "/var/task/data/sample")))
