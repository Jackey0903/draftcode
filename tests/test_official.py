...
def test_normalizer_outputs_engine_ready_files(tmp_path: Path) -> None:
    ...
    # Pool is the full official answer-card pool (球员池_勿修改, 124). Players the
    # market never rates are sunk later by `draftcode rerank`, not filtered here.
...
def test_pool_is_full_official_pool(tmp_path: Path) -> None:
    out_dir = tmp_path / "processed"
    ingest_official(SOURCE_DIR, out_dir, use_llm_divergence=False)
    rows = _read_csv(out_dir / "prospects.csv")
    by_id = {row["prospect_id"]: row for row in rows}

    # Full official answer-card pool (球员池_勿修改): all 124 entries are kept;
    # the rerank step (not ingest) decides who reaches the top-30.
    assert len(rows) == 124
    assert "p084" in by_id and by_id["p084"]["school"] == "田纳西大学"
    # Every player carries the columns needed for unified talent+market+odds ranking.
    for column in ("talent_signal", "market_rank", "odds_signal", "consensus_rank"):
